...
public class CuratorClient {
    ...
    // TODO: extract these serialization operations to a new class
    /**
     ...
     *
     * @param data A Thrift data structure object to serialize
     * @return A string version of the input data structure
     * @throws TException
     */
    private static byte[] getSerializedThriftStructure(TBase data)
            throws TException {
    ...
    private static void getThriftStructureFromString( TBase inOutBaseObject,
        ...
        deserializer.deserialize( inOutBaseObject, serializedData );
    }
	 ...
	 */
	public static Map<String, byte[]> serializeRecord(Record record)
            throws TException, IOException {
        Map<String, byte[]> serializedForm = new HashMap<String, byte[]>();
        byte[] raw = record.getRawText().getBytes();
        serializedForm.put("original", raw);
        serializedForm.put("hash", record.getIdentifier().getBytes());
        ...
        for( String key : allViews.keySet() ) {
            ...
            byte[] serializedLabeling = serializer.getSerializedForm( view );
     ...
     *            #serializeRecord()
     */
	public static Record deserializeRecord( Map<String, byte[]> map ) throws TException {
        ...
        String raw = Arrays.toString( map.get( "original" ) );
        String hash = Arrays.toString( map.get( "hash" ) );
    ...
    public static Record deserializeRecord( File inputDirectory )
        ...
        Map<String, byte[]> serialForm = new HashMap<String, byte[]>();
        ...
        for( File f : inputDirectory.listFiles() ) {
            ...
            String annotationType = getAnnotationTypeFromFileName( f.getName() );

            serialForm.put( annotationType, readFileToBytes(f) );
        }
    ...
    public static void addRecordsFromJobDirectory(File jobDir, boolean checkdb)
        ...
        for (File docDir : jobDir.listFiles()) {
            ...
            if( currentRecord == null ) {
                ...
                for (File annotationFile : docDir.listFiles()) {
                    Map<String, byte[] > annotations =
                            new HashMap<String, byte[]>();

                    try {
                        byte[] annotationContents = readFileToBytes(annotationFile);
                        String annotationType = getAnnotationTypeFromFileName(
                                annotationFile.getName() );

                        // Make sure this isn't an operating system file
                        if( !annotationFile.isHidden() ) {
                            annotations.put( annotationType, annotationContents );
                        }

                        currentRecord = deserializeRecord( annotations );
                    }
                    catch (IOException e) {
                        e.printStackTrace();
                    } // END try/catch
    ...
    // TODO: Extract these file system operations to a new class
    ...
    public static void writeSerializedRecords( File outputDir )
        ...
        for( Record r : newInputRecords ) {
            ...
            Map< String, byte[] > serializedForm = serializeRecord( r );
     ...
     *                     exists.
     */
    private static void writeFile( File path, byte[] text )
            throws IOException {
        if( path.exists() ) {
     ...
     */
    private static synchronized void testPOSAndTokenizer(TTransport transport)
        ...
        for( Record r : newInputRecords ) {
            ...
            getThriftStructureFromString( reconstructed,
                                          getSerializedThriftStructure(r) );
