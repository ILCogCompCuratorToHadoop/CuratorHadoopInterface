...
public class CuratorHandler implements Curator.Iface {
	...
	private Record getRecord(String text, boolean whitespaced)
		...
		if (text.trim().equals("")) {
			throw new AnnotationFailedException("Cannot annotate the empty string");
		}
        ...
        // If record not found in archive . . .
		if (record == null) {
			if (slave) {
			...
			} else if (record == null) { // No record found; set up a blank one
...
// 	}
	...
	public void performAnnotation(Record record, String view_name,
        ...
        // Update the time of the last annotation
        lastAnnotationTime = System.currentTimeMillis();
	}
